...
export default function ProfileContent({ userId }: { userId: string }) {
  ...
  const connectToYouTube = () => {
    ...
    const callbackUrl = `${window.location.origin}/profile?connect=youtube&t=${timestamp}`;
          ...
          <div className="flex items-center justify-between">
            ...
            {youtubeConnected ? (
              ...
              <button
                ...
                disabled={isConnecting}
                className="px-3 py-1 bg-red-700 hover:bg-red-600 text-white rounded-md text-sm transition-colors shadow-[0_0_5px_#ff0000] crt-glow-youtube disabled:opacity-50"
              >
                {isConnecting ? 'Connecting...' : 'Connect'}
              </button>
            )}
